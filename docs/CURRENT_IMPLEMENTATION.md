...
| **Business Home (`/business/home`)** | Three-variant operational banner using existing local imagery with previous/next arrows and reduced-motion pause; 16:9 desktop banner-left/direct-actions-right, compact 2:1 tablet and square mobile compositions; attention, next work and honest Booking/Grooming/Hotel summaries; unsupported revenue is absent | Implemented local prototype |
| **Business Calendar (BF-2)** | Sunday-first Day/Week/Month/Custom views; 28/35/42-day custom grids; Agenda-first mobile; continuous Hotel spans; time-positioned Grooming; touch-safe 44px resize handles; Ctrl/Cmd copy/paste and Alt-drag duplication; progressive interaction/shortcut legend; view preference cookie; availability-guarded mutation without layout-shifting success banners | Implemented local prototype |
...
| **Hotel / Boarding Operations Foundation (BF-6)** | `/business/hotel` capability-gated dashboard; desktop/tablet retain Pet-first continuous room/zone occupancy; mobile uses `วันนี้` / `กำลังพัก` / `เข้าพัก` / `ออกวันนี้` / `งานดูแล` task tabs and never renders the occupancy grid; room assignment/move history, conflict guards, per-Pet care, checkout history and Stay detail controls remain intact | Browser-local shared Business envelope; a distinct `PrototypeHotelStay` slice references Booking/Customer/Pet/Branch/Resource records and is not a Booking-status or generic Service Job migration |
| **Customers & Pets (BF-3)** | Responsive operational rows with single-boundary search and actionable filters; concise Customer header/detail hierarchy; neutral Customer/Pet avatars; multiple-Pet snap/peek summaries on mobile; progressive Passport/access disclosure; local add Customer/Pet, tags/notes, duplicate warning and Booking preselection | Browser-local session storage; no Customer/Guardian authority backend |
| **Inbox & Customer Communication (BF-4/BF-6 boundary)** | `/business/inbox` readable desktop split/mobile full-task UI; Business-wide Customer conversation reuse; single-boundary search; compact filters; safe Customer/Pet/Booking/Branch/Grooming context; local send/unread; horizontal mobile quick replies; query recovery and structured add-service request; no staff-facing Guardian simulator | Separate same-tab session storage; a Guardian-response helper remains only in local state/tests and can affect the linked Grooming estimate/add-on; no real messaging, attachments, notifications, Consumer Inbox, authorization, Booking, Hotel Stay, Charge, or Payment effect |
| **Shared Intake & Scanner (Phase E/BF-6 handoff)** | Camera scan, neutral manual code recovery, QR validation, consent review, belongings logging, check-in completion, eligible Grooming Job handoff and valid Hotel Stay/room-target handoff; fixture/interruption controls remain state/test-only | Implemented local prototype |
...
| **Shared Business visual primitives** | Semantic buttons/forms/badges/status, workflow-appropriate list/row/timeline/board surfaces, 44px touch controls, adaptive dialog/sheet treatment, progressive disclosure, canonical motion tokens, reduced-motion fallbacks and structural loading skeletons | Visual migration only; existing Product state and domain behavior preserved |
...
- The 2026-08-30 Business UX/UI reset audited the public Business entry, shell, Home, Calendar/Booking overlays, Customers/detail, Inbox, Scanner/Intake, Grooming and Hotel across 320/375/390/430/768/820/1024/1200/1440px. It removed redundant operational overview copy and staff-facing development controls, established 16px body typography and 44px touch targets, and preserved BF1–BF6 behavior and authority boundaries.
