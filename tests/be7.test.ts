...
import { PostgresBe7Repository } from "../app/_backend/be7/postgresRepository";
...
import { ARI, INACTIVE, MANAGER, metadata, OUTSIDER, OWNER, seededDatabase, STAFF, THONGLOR, WHISKER } from "./postgresFixtures";
...
async function fixture() {
  ...
  db.inspect.prepare("INSERT INTO payment_provider_accounts(id,business_id,provider,external_account_id,secret_ref,source) VALUES(?,?,'mock','TEST-MERCHANT','test-secret-reference','dev-test')").run(accountId, WHISKER);
  const provider = new MockPaymentProvider("test-only-payment-secret", now), resolver = async () => provider, repo = new PostgresBe7Repository(db, "dev-test"), app = new Be7Application(new PostgresBe1Repository(db), repo, resolver, now);
  const run = async (op: Be7Operation, actor = OWNER) => app.executeBe7(await app.resolvePerson(actor), op, metadata());
  const source = db.inspect.prepare("SELECT id FROM service_executions WHERE booking_id='booking-fixture-ari-grooming-1030'").get()!.id as string;
...
test("checkout creates one canonical Booking Charge; concurrent same-payment retries allocate once and completion is independent", async (t) => {
  ...
  assert.equal(one.balance.status, "unpaid"); assert.equal(f.db.inspect.prepare("SELECT count(*) n FROM payments").get()?.n, 0);
  const before = f.db.inspect.prepare("SELECT * FROM service_executions").all();
  ...
  assert.deepEqual(f.db.inspect.prepare("SELECT * FROM service_executions").all(), before); assert.deepEqual(f.db.inspect.prepare("SELECT conname FROM pg_constraint WHERE connamespace=current_schema()::regnamespace AND contype='f' AND NOT convalidated").all(), []);
});
test("competing payment, discount, and cancellation respect revision and debt constraints atomically", async (t) => {
  const f = await fixture(); t.after(() => f.db.inspect.close()); const { balance: b } = await f.checkout();
  ...
  assert.equal(f.db.inspect.prepare("SELECT count(*) n FROM payments").get()?.n, 1);
});
test("financial policy enforces Staff collection, Manager adjustment/refund, Branch access and write-time role changes", async (t) => {
  ...
  const membership = f.db.inspect.prepare("SELECT id FROM business_memberships WHERE person_id=? AND business_id=?").get(STAFF, WHISKER)!.id;
  f.db.inspect.prepare("INSERT INTO membership_branch_access(membership_id,business_id,branch_id,status,created_at,updated_at) VALUES(?,?,?,'active',?,?)").run(membership!, WHISKER, ARI, f.now(), f.now());
  ...
  const write = f.repo.write.bind(f.repo); f.repo.write = async (...args) => { f.db.inspect.prepare("UPDATE business_memberships SET role='STAFF' WHERE person_id=? AND business_id=?").run(MANAGER, WHISKER); return write(...args); };
...
test("partial and full refunds are immutable, idempotent and do not cancel debt or completed work", async (t) => {
  const f = await fixture(); t.after(() => f.db.inspect.close()); const { balance: b } = await f.checkout(); await f.pay(b, b.total);
  const paymentId = f.db.inspect.prepare("SELECT id FROM payments").get()!.id as string, paid = await f.get(b.charge.chargeId);
  ...
  assert.throws(() => f.db.inspect.exec("DELETE FROM payments"), /BE7_SCOPE/);
});
test("persistence failure rolls back Payment, allocation, audit, receipt and revision before safe retry", async (t) => {
  const f = await fixture(); t.after(() => f.db.inspect.close()); const { balance: b } = await f.checkout();
  f.db.inspect.exec("CREATE FUNCTION test_payment_failure_fn() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'test-payment-failure'; END $$; CREATE TRIGGER test_payment_failure BEFORE INSERT ON payment_allocations FOR EACH ROW EXECUTE FUNCTION test_payment_failure_fn()");
  await assert.rejects(f.pay(b, 100, "payment-rollback-key"), failure("PERSISTENCE_ERROR"));
  assert.equal(f.db.inspect.prepare("SELECT count(*) n FROM payments").get()?.n, 0); assert.equal((await f.get(b.charge.chargeId)).charge.revision, b.charge.revision);
  f.db.inspect.exec("DROP TRIGGER test_payment_failure ON payment_allocations"); await f.pay(b, 100, "payment-rollback-key");
});
test("provider-neutral Attempt reserves outstanding money, deduplicates signed webhooks and separates provider success from service status", async (t) => {
  const f = await fixture(); t.after(() => f.db.inspect.close()); const { balance: b } = await f.checkout(); const reserved = await f.attempt(b);
  ...
  assert.equal((await f.get(b.charge.chargeId)).status, "paid"); assert.equal(f.db.inspect.prepare("SELECT count(*) n FROM payments").get()?.n, 1);
...
test("an ambiguous provider result reconciles the original intent and concurrent workers cannot collect twice", async (t) => {
  const f = await fixture(); t.after(() => f.db.inspect.close()); const { balance: b } = await f.checkout(); const a = (await f.attempt(b)).balance.attempts[0];
...
test("late provider receipt after terminal failure preserves actual money as unallocated reconciliation", async (t) => {
  ...
  assert.equal(f.db.inspect.prepare("SELECT count(*) n FROM payment_webhook_events WHERE failure_code='unallocated-payment'").get()?.n, 1);
});
...
test("merchant rotation during signature verification fails closed before settlement", async (t) => {
  ...
  f.provider.verify = async (...args) => { const valid = await verify(...args); f.db.inspect.prepare("UPDATE payment_provider_accounts SET secret_ref='rotated-secret',revision=revision+1 WHERE id=?").run(accountId); return valid; };
  ...
  assert.equal(f.db.inspect.prepare("SELECT count(*) n FROM payments").get()?.n, 0);
  assert.equal(f.db.inspect.prepare("SELECT count(*) n FROM payment_webhook_events").get()?.n, 0);
});
...
test("duplicate provider reference and mismatched late event create review evidence without duplicate money or renewed reservation", async (t) => {
  const f = await fixture(); t.after(() => f.db.inspect.close()); const { balance: b } = await f.checkout(); const first = (await f.attempt(b, 100)).balance.attempts[0];
  ...
  assert.equal(f.db.inspect.prepare("SELECT count(*) n FROM payment_webhook_events WHERE failure_code='duplicate-provider-reference'").get()?.n, 1);
  ...
  assert.throws(() => f.db.inspect.prepare("UPDATE payment_attempts SET state='reconciliation' WHERE id=?").run(second.attemptId), /BE7_AMOUNT/);
});
