# Supabase target acceptance

EXTERNAL CONFIG REQUIRED / NOT YET DEPLOYED. Follow the [runbook](./PRODUCTION_RUNBOOK.md); do not create another Supabase project or use real customer data before the checkpoint is approved.

1. Engineer configures the selected Supabase/Cloudflare environment and applies all four migrations to an empty database only after the checkpoint is approved.
...
8. Complete the previously documented BE1/BE2 stale-form/retry follow-up and load/latency/backup/restore acceptance before production.
