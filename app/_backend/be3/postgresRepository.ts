...
export class PostgresBe3Repository implements Be3Repository {
...
`).bind(
    ...
    for (const scope of scopes.values()) statements.push(...new PostgresBe4Repository(this.database).catalogStatements(scope.businessId, scope.branchId, occurredAt, actorPersonId));
  ...
  async listOverlappingBookings(businessId: string, branchId: string, startMinute: number, endMinute: number, excludeBookingId?: string) {
    const rows = await this.database.prepare(`${this.bookingSelect()}
      WHERE booking.business_id = ? AND booking.branch_id = ?
        ...
        AND (?::text IS NULL OR booking.id <> ?)
      ORDER BY booking.start_minute, booking.id
  ...
  async createBooking(write: BookingPersistenceWrite, context: AuthorizedMutation) {
    ...
    const statements: PreparedStatement[] = [
      ...
      ...await new PostgresBe4Repository(this.database).bookingStatements(booking, context),
    ];
    try {
      const results = await this.database.batch(statements);
      results.forEach(assertSuccess);
  ...
  async replaceBooking(before: BookingView, write: BookingPersistenceWrite, action: string, context: AuthorizedMutation) {
    ...
    const statements: PreparedStatement[] = [
      update,
      this.database.prepare("INSERT INTO backend_guards(id,allowed,version_ok) SELECT ?,1,(:previous_row_count::integer=1)::integer").bind(`booking-${context.metadata.requestId}`),
  ...
  private bookingWhere(query: BookingQuery) {
    ...
    conditions.push("booking.branch_id IN (SELECT value FROM jsonb_array_elements_text((?)::text::jsonb))");
    ...
    if (query.modules.length) {
      conditions.push("booking.service_module IN (SELECT value FROM jsonb_array_elements_text((?)::text::jsonb))");
      bindings.push(JSON.stringify(query.modules));
    }
    if (query.statuses.length) {
      conditions.push("booking.status IN (SELECT value FROM jsonb_array_elements_text((?)::text::jsonb))");
  ...
  private async hydrateServices(businessId: string, branchId: string, rows: ServiceRow[]) {
    ...
    const requirements = await this.database.prepare(`
      ...
      WHERE business_id = ? AND branch_id = ?
        AND service_id IN (SELECT value FROM jsonb_array_elements_text((?)::text::jsonb))
  ...
  private async hydrateResources(businessId: string, branchId: string, rows: ResourceRow[]) {
    ...
    const [links, windows] = await Promise.all([
      this.database.prepare(`
        ...
        WHERE business_id = ? AND branch_id = ?
          AND resource_id IN (SELECT value FROM jsonb_array_elements_text((?)::text::jsonb))
    ...
    const staff = new Map((await new PostgresBe4Repository(this.database).staff(businessId, branchId)).map((s) => [s.staffId, s]));
    ...
    const [pets, assignments, requirements] = await Promise.all([
      this.database.prepare(`
        ...
        WHERE link.business_id = ?
          AND link.booking_id IN (SELECT value FROM jsonb_array_elements_text((?)::text::jsonb))
      ...
      this.database.prepare(`
        ...
        WHERE business_id = ?
          AND booking_id IN (SELECT value FROM jsonb_array_elements_text((?)::text::jsonb))
  ...
  private petStatements(write: BookingPersistenceWrite, context: AuthorizedMutation, conditional: boolean) {
    ...
    const insert = `
      ...
      SELECT ?, ?, ?, ((value)::jsonb->>'id'), ((value)::jsonb->>'position')::integer, ?, ?
  ...
  private reservationStatements(write: BookingPersistenceWrite, conditional: boolean, writeToken?: string) {
    ...
    // Expand reservations in one statement; every row runs the PostgreSQL
    // conflict triggers inside the same serializable transaction.
    ...
    const insert = `
      ...
      SELECT ?, ?, ?,
        ((value)::jsonb->>'resourceId'),
        ((value)::jsonb->>'reservationKey'),
        ((value)::jsonb->>'reservationDate'),
        ((value)::jsonb->>'startMinute')::integer,
        ((value)::jsonb->>'endMinute')::integer,
        ((value)::jsonb->>'units')::integer
      FROM jsonb_array_elements_text((?)::text::jsonb)
    `;
    return [conditional
      ? this.database.prepare(`${insert}
