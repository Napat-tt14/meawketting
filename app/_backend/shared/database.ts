...
export function authorizationGuard(db: D1DatabaseLike, context: AuthorizedMutation, branchId: string, token: string, ownerOnly = false) {
  return db.prepare(`INSERT INTO backend_guards(id, allowed)
    SELECT ?, EXISTS(SELECT 1 FROM persons p JOIN business_memberships m ON m.person_id=p.id
      ...
      WHERE p.id=? AND p.status='active' AND m.id=? AND m.business_id=? AND m.status='active'
        ...
        AND (m.role='OWNER' OR EXISTS(SELECT 1 FROM membership_branch_access a
          WHERE a.business_id=m.business_id AND a.membership_id=m.id AND a.branch_id=br.id AND a.status='active')))`)
    .bind(token, context.actor.id, context.membership.id, context.membership.businessId, branchId, ownerOnly ? 1 : 0);
}

export async function findReceipt(db: D1DatabaseLike, businessId: string, command: string, requestKey: string) {
...
export function receiptStatement(db: D1DatabaseLike, context: AuthorizedMutation, branchId: string, receipt: MutationReceipt, predicate = "1", bindings: unknown[] = []) {
...
export function auditStatement(db: D1DatabaseLike, context: AuthorizedMutation, branchId: string, action: string, targetType: string, targetId: string, before: unknown, after: unknown, predicate = "1", bindings: unknown[] = []) {
...
export function deleteGuard(db: D1DatabaseLike, token: string) { return db.prepare("DELETE FROM backend_guards WHERE id=?").bind(token); }

export async function batch(db: D1DatabaseLike, statements: D1PreparedStatementLike[]) {
  try {
    ...
    if (message.includes("UNIQUE constraint failed: business_intakes")) conflict("version-conflict");
    if (message.includes("UNIQUE constraint failed: conversations")) conflict("version-conflict");
    if (message.includes("UNIQUE constraint failed: channel_webhook_events")) conflict("idempotency");
    ...
    if (message.includes("UNIQUE constraint failed: charges") || message.includes("UNIQUE constraint failed: charge_items")) conflict("version-conflict");
    if (message.includes("UNIQUE constraint failed: payment_webhook_events") || message.includes("UNIQUE constraint failed: payments") || message.includes("UNIQUE constraint failed: payment_attempts")) conflict("idempotency");
    ...
    if (message.includes("UNIQUE constraint failed: backend_mutations")) conflict("idempotency");
