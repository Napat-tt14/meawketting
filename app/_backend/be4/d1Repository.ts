...
export class D1Be4Repository implements Be4Repository {
  ...
  catalogStatements(businessId: string, branchId: string, now: string, actorId: string) {
    ...
    return [
      db.prepare(`INSERT INTO booking_resources(id,business_id,branch_id,module,kind,label,capacity_mode,capacity,compatibility_staff_id,status,created_at,updated_at,created_by_person_id,updated_by_person_id)
        SELECT 'resource_'||lower(hex(randomblob(16))),s.business_id,a.branch_id,'grooming','groomer',s.name,'exclusive',1,s.id,'active',?,?,?,?
        FROM operation_staff s JOIN operation_staff_branches a ON a.business_id=s.business_id AND a.staff_id=s.id
        WHERE a.business_id=? AND a.branch_id=? AND s.status='active' AND EXISTS(SELECT 1 FROM json_each(s.capabilities_json) WHERE value='grooming')
      ...
      db.prepare(`INSERT OR IGNORE INTO booking_resource_service_links(business_id,branch_id,resource_id,service_id,created_at,created_by_person_id)
        ...
        WHERE r.business_id=? AND r.branch_id=? AND r.compatibility_staff_id IS NOT NULL`).bind(now, actorId, businessId, branchId),
    ];
  }
  ...
  async saveStaff(before: StaffView | null, after: StaffView, context: AuthorizedMutation, branchId: string, receipt: MutationReceipt) {
    ...
    statements.push(db.prepare("INSERT INTO backend_guards(id,allowed,version_ok) SELECT ?,1,changes()=1").bind(token));
  ...
  async correctRecord(before: ServiceRecordView, after: ServiceRecordView, context: AuthorizedMutation, receipt: MutationReceipt) {
    ...
    await batch(db, [authorizationGuard(db, context, after.branchId, token),
      ...
      db.prepare("INSERT INTO backend_guards(id,allowed,version_ok) SELECT ?,1,changes()=1").bind(`${token}-version`),
  ...
  private async hydrate(rows: ExecutionRow[]) {
    ...
    const scope = "business_id=? AND branch_id=? AND execution_id IN (SELECT value FROM json_each(?))";
    const [assignments, events, tasks] = await Promise.all([
      ...
      this.rows<EventRow>(`SELECT * FROM execution_events WHERE ${scope} ORDER BY occurred_at,rowid`, businessId, branchId, keys),
  ...
  async staff(businessId: string, branchId: string): Promise<StaffView[]> {
    ...
    const [links, windows] = await Promise.all([
      this.rows<{ staff_id: string; branch_id: string }>("SELECT * FROM operation_staff_branches WHERE business_id=? AND staff_id IN (SELECT value FROM json_each(?))", businessId, ids),
      this.rows<{ id: string; staff_id: string; state: StaffView["availability"][number]["state"]; start_local: string; end_local: string; note: string | null }>("SELECT * FROM operation_staff_windows WHERE business_id=? AND staff_id IN (SELECT value FROM json_each(?)) ORDER BY id", businessId, ids),
    ]);
  ...
  async records(businessId: string, branchId: string, executionIds?: string[]): Promise<ServiceRecordView[]> {
    const rows = await this.rows<{ id: string; execution_id: string; booking_id: string; module: ExecutionView["kind"]; customer_id: string; pet_id: string; completed_at: string; summary: string; business_note: string; snapshot_json: string; revision: number; created_at: string; updated_at: string }>(
      `SELECT r.*,e.booking_id,e.module FROM service_records r JOIN service_executions e ON e.business_id=r.business_id AND e.branch_id=r.branch_id AND e.id=r.execution_id WHERE r.business_id=? AND r.branch_id=? ${executionIds ? "AND r.execution_id IN (SELECT value FROM json_each(?))" : ""} ORDER BY r.completed_at,r.id`,
      businessId, branchId, ...(executionIds ? [JSON.stringify(executionIds)] : []));
    const revisions = await this.rows<{ id: string; record_id: string; kind: string; data_json: string }>("SELECT * FROM service_record_revisions WHERE business_id=? AND branch_id=? AND record_id IN (SELECT value FROM json_each(?)) ORDER BY occurred_at,id", businessId, branchId, JSON.stringify(rows.map((r) => r.id)));
  ...
  statements(write: ExecutionWrite, token: string): D1PreparedStatementLike[] {
    ...
    statements.push(db.prepare(`INSERT INTO backend_guards(id,allowed,version_ok) SELECT ?,1,${predicate}`).bind(`${token}-version`, ...bindings));
    if (write.requireActiveSource) {
      statements.push(db.prepare(`INSERT INTO backend_guards(id,allowed,version_ok) SELECT ?,1,EXISTS(
        ...
        WHERE b.business_id=? AND b.branch_id=? AND b.id=? AND b.status<>'cancelled' AND c.status='active' AND p.status='active')`)
    ...
    for (const s of write.checkedStaff ?? []) {
      statements.push(db.prepare(`INSERT INTO backend_guards(id,allowed,version_ok) SELECT ?,1,EXISTS(SELECT 1 FROM operation_staff WHERE business_id=? AND id=? AND revision=? AND status='active')`)
  ...
  private recordStatements(record: ServiceRecordView, execution: ExecutionView, context: AuthorizedMutation, predicate: string, bindings: unknown[]) {
    ...
    return [
      db.prepare(`INSERT INTO service_record_revisions(id,business_id,branch_id,record_id,kind,data_json,actor_person_id,occurred_at)
        SELECT ?,business_id,branch_id,id,'source-recompleted',json_object('id',?,'at',?,'completedAt',completed_at,'summary',summary,'businessNote',business_note,'details',json_extract(snapshot_json,'$.details'),'activities',json_extract(snapshot_json,'$.activities'),'staffResourceLabels',json_extract(snapshot_json,'$.staffResourceLabels'),'photos',json('[]'),'reason','source-recompleted'),?,?
  ...
  async save(write: ExecutionWrite, additionalStatements: D1PreparedStatementLike[] = []) {
