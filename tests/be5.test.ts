...
import { D1Be5Repository } from "../app/_backend/be5/d1Repository";
...
import { ARI, INACTIVE, metadata, OUTSIDER, OWNER, seededDatabase, THONGLOR, WHISKER } from "./backendTestKit";
...
async function fixture() {
  const db = seededDatabase(); db.sqlite.exec(readFileSync("scripts/seed-be5-dev.sql", "utf8"));
  let time = "2050-08-18T02:00:00.000Z";
  const now = () => time, auth = new D1Be1Repository(db), repository = new D1Be5Repository(db, "dev-test"), guardian = new GuardianGrantService(db, "dev-test", now), app = new Be5Application(auth, repository, now), actor = await app.resolvePerson(OWNER);
...
test("Temporary QR uses opaque entropy, safe hashes and explicit Primary Guardian authority independent from Customer", async (t) => {
  ...
  const stored = f.db.sqlite.prepare("SELECT * FROM access_grants").all(); assert.equal(JSON.stringify(stored).includes(issued.token!), false);
...
test("pending Consent reveals no Pet, then only approved fields; correction retains source and receive links canonical Grooming", async (t) => {
  ...
  assert.equal(f.db.sqlite.prepare("SELECT name FROM passport_profiles WHERE pet_id=?").get(input.petId)?.name, "Mochi Passport");
  ...
  const execution = f.db.sqlite.prepare("SELECT * FROM service_executions WHERE id=?").get(result.record.serviceJobId!);
  assert.equal(execution?.intake_id, result.record.id); assert.equal(execution?.status, "checked-in");
  assert.equal(f.db.sqlite.prepare("SELECT status FROM bookings WHERE id=?").get(execution!.booking_id)?.status, "confirmed");
  assert.equal(f.db.sqlite.prepare("SELECT count(*) n FROM service_records").get()?.n, 0);
...
test("Business and Branch isolation, forged targets, and unknown Pet receipt never manufacture a relationship or service", async (t) => {
  const f = await fixture(); t.after(() => f.db.sqlite.close()); const issued = await f.issue();
  ...
  const foreign = f.db.sqlite.prepare("SELECT id FROM service_executions WHERE branch_id=? LIMIT 1").get(THONGLOR);
  ...
  assert.equal(f.db.sqlite.prepare("SELECT count(*) n FROM business_pet_profiles WHERE pet_id='pet_be5_unlinked_000001'").get()?.n, 0);
});
...
test("concurrent starts converge on one Intake and stale edits conflict; revocation during receive rolls back the whole handoff", async (t) => {
  ...
  f.repository.write = async (...args) => {
    f.db.sqlite.prepare("UPDATE access_grants SET revoked_at=?,revision=revision+1 WHERE id=?").run("2050-08-18T02:01:00.000Z", issued.grantId);
    return write(...args);
  };
  ...
  assert.equal(f.db.sqlite.prepare("SELECT checked_in_at FROM business_intakes WHERE id=?").get(one.record.id)?.checked_in_at, null);
  assert.equal(f.db.sqlite.prepare("SELECT count(*) n FROM service_executions WHERE intake_id=?").get(one.record.id)?.n, 0);
  assert.equal(f.db.sqlite.prepare("SELECT count(*) n FROM backend_guards").get()?.n, 0);
});
...
test("Guardian decisions replay concurrent identical requests, reject changed payloads and recheck expiry at commit", async (t) => {
  const f = await fixture(); t.after(() => f.db.sqlite.close()); const issued = await f.issue({ pending: true });
  await Promise.all([1, 2].map(() => f.guardian.decide(GUARDIAN, WHISKER, ARI, issued.grantId, "approved", 1, "same-approval")));
  assert.equal(f.db.sqlite.prepare("SELECT count(*) n FROM access_events WHERE grant_id=? AND kind='approved'").get(issued.grantId)?.n, 1);
  ...
  assert.equal(f.db.sqlite.prepare("SELECT status FROM consents WHERE id=(SELECT consent_id FROM access_grants WHERE id=?)").get(expired.grantId)?.status, "pending");
});
