...
import { D1Be6Repository } from "../app/_backend/be6/d1Repository";
...
import { ARI, INACTIVE, MANAGER, metadata, OUTSIDER, OWNER, seededDatabase, THONGLOR, WHISKER } from "./backendTestKit";
...
async function fixture() {
  const db = seededDatabase(); for (const n of [5, 6]) db.sqlite.exec(readFileSync(`scripts/seed-be${n}-dev.sql`, "utf8"));
  let time = "2050-08-18T02:00:00.000Z"; const now = () => time, repo = new D1Be6Repository(db, "dev-test"), app = new Be6Application(new D1Be1Repository(db), repo, now), actor = await app.resolvePerson(OWNER);
...
test("one Business/Customer Conversation survives concurrent creation; messages order and retry once with durable outbox", async (t) => {
  ...
  assert.equal(f.db.sqlite.prepare("SELECT count(*) n FROM message_outbox").get()?.n, 3);
  assert.equal(f.db.sqlite.prepare("SELECT count(*) n FROM audit_events WHERE action='message.send'").get()?.n, 3);
});
...
test("Business-wide identity preserves per-message Branch privacy and rejects forged context, inactive members and foreign Customers", async (t) => {
  const f = await fixture(); t.after(() => f.db.sqlite.close()); const c = await f.ensure(); await f.send(c.conversationId, "อารีย์");
  ...
  const write = f.repo.write.bind(f.repo); f.repo.write = async (...args) => { f.db.sqlite.prepare("UPDATE business_memberships SET status='inactive' WHERE person_id=? AND business_id=?").run(OWNER, WHISKER); return write(...args); };
...
test("signed incoming events deduplicate, retain unknown contacts without inferring identity, and derive per-Person unread cursors", async (t) => {
  ...
  assert.equal(f.db.sqlite.prepare("SELECT count(*) n FROM channel_webhook_events WHERE state='unlinked'").get()?.n, 1);
  assert.equal(f.db.sqlite.prepare("SELECT count(*) n FROM conversations").get()?.n, 1);
  assert.equal(f.db.sqlite.prepare("SELECT count(*) n FROM person_external_identities").get()?.n, 0);
...
test("outbox lease prevents double dispatch, retries unchanged provider intent and exposes missing channels honestly", async (t) => {
  const f = await fixture(); t.after(() => f.db.sqlite.close()); const c = await f.ensure(); await f.send(c.conversationId, "รับกลับได้แล้ว");
  f.adapter.failures.push({ state: "retry", code: "test-network" }); await f.transport.drain();
  const row = f.db.sqlite.prepare("SELECT * FROM message_outbox").get()!; assert.equal(row.state, "retry"); assert.equal(row.attempts, 1);
  f.setTime("2050-08-18T02:02:00.000Z"); await Promise.all([f.transport.drain(), f.transport.drain()]);
  assert.equal(f.adapter.accepted.size, 1); assert.equal(f.db.sqlite.prepare("SELECT state FROM message_outbox").get()?.state, "sent");
  assert.deepEqual(f.db.sqlite.prepare("SELECT outcome FROM outbox_attempts ORDER BY started_at").all().map((r) => r.outcome), ["retry", "accepted"]);
  ...
  f.db.sqlite.prepare("UPDATE business_channels SET state='disconnected' WHERE business_id=?").run(WHISKER);
  await f.send(c.conversationId, "รอเชื่อมช่องทาง");
  assert.equal(f.db.sqlite.prepare("SELECT count(*) n FROM message_outbox WHERE state='blocked'").get()?.n, 1);
});

test("Guardian-only structured approval updates one Job atomically and never the Booking or financial state", async (t) => {
  const f = await fixture(); t.after(() => f.db.sqlite.close()); const c = await f.ensure();
  ...
  const booking = f.db.sqlite.prepare("SELECT * FROM bookings WHERE id=?").get(value.bookingId!);
  await Promise.all([1, 2].map(() => guardian.decide(GUARDIAN, WHISKER, ARI, m.messageId, "approved", 1, "guardian-decision-key")));
  assert.equal(f.db.sqlite.prepare("SELECT count(*) n FROM execution_events WHERE execution_id=? AND kind='addon'").get(response.serviceJobId!)?.n, 1);
  assert.deepEqual(f.db.sqlite.prepare("SELECT * FROM bookings WHERE id=?").get(value.bookingId!), booking);
...
test("failed approval side effects roll back the decision and a safe retry commits once", async (t) => {
  ...
  f.db.sqlite.exec("CREATE TRIGGER test_approval_rollback BEFORE INSERT ON execution_events WHEN NEW.kind='addon' BEGIN SELECT RAISE(ABORT,'test-unavailable'); END");
  ...
  assert.equal(f.db.sqlite.prepare("SELECT status FROM message_approvals WHERE message_id=?").get(m.messageId)?.status, "waiting");
  f.db.sqlite.exec("DROP TRIGGER test_approval_rollback");
  await guardian.decide(GUARDIAN, WHISKER, ARI, m.messageId, "approved", 1, "retry-approval-atomic");
  assert.equal(f.db.sqlite.prepare("SELECT status FROM message_approvals WHERE message_id=?").get(m.messageId)?.status, "approved");
});

test("ambiguous delivery stops for reconciliation when provider retry protection has expired", async (t) => {
  const f = await fixture(); t.after(() => f.db.sqlite.close()); const c = await f.ensure(); await f.send(c.conversationId, "retry protection");
  ...
  assert.equal(f.db.sqlite.prepare("SELECT failure_code FROM message_outbox").get()?.failure_code, "reconciliation-required");
});
