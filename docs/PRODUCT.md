...
1. **Business Core**:
   ...
   - Shared Business Intake & Scanner (`/business/scan`, `/business/intake/[intakeId]`)
    - Customers & Pets (BF-3 frozen UI backed by BE2 durable Business-scoped data), Inbox & Customer Communication (BF-4 / BE6 PostgreSQL), Grooming Operations (BF-5 / BE4 PostgreSQL), Hotel / Boarding Operations (BF-6 / BE4 PostgreSQL), Billing / Payments / Revenue (BF-7 / BE7 PostgreSQL), Service Record & Reports foundation (BF-8 / BE4 + BE8), and Team & Staff Operations (BF-9 / BE4 PostgreSQL at `/business/team`)
2. **Service Modules**:
...
- **Business Authentication**: Supabase Auth through Google or LINE; `/business/register` permits a verified new user to create their first store only after submitting store details and explicitly confirming authority to manage it. OAuth login alone creates no Owner. Contact details are self-declared, not legal-business verification.
