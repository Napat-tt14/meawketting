...
export function HotelStayDetail({
  ...
  function advanceLifecycle() {
    ...
    if (stay.status === "booked" || stay.status === "expected-today") {
      ...
      result = transitionNotice(transitionPrototypeHotelStay(stay.hotelStayId, "checked-out", context), "เช็กเอาต์แล้ว · บันทึกประวัติบริการล่าสุดไว้แล้ว");
    } else {
        ...
        <div className="hotel-stay-detail__content">
          ...
          <section className="hotel-detail-section" aria-labelledby="hotel-care-title">
            ...
            {stay.dailyCareTasks.length > 0 ? <ul className="hotel-care-list">{stay.dailyCareTasks.map((task) => {
              ...
              return <li key={task.id} className={task.state === "completed" ? "is-complete" : ""}><span className="hotel-care-list__time"><time>{task.scheduledTime}</time><small>{hotelCareTaskIconLabel(task)}</small></span><span className="hotel-care-list__task"><strong>{task.label}</strong><small>{task.completedAt ? `โดย ${task.completedBy ?? "ทีมร้าน"}` : stateLabel}</small></span><button type="button" disabled={task.state === "completed"} onClick={() => completeCare(task.id)} aria-label={`${task.state === "completed" ? "ทำเสร็จแล้ว" : "ทำเครื่องหมายเสร็จ"} ${task.label}`}>{task.state === "completed" ? <CheckCircle size={18} /> : "เสร็จ"}</button></li>;
          ...
          <section className="hotel-detail-section" aria-labelledby="hotel-instructions-title">
            ...
            <div className="hotel-detail-instructions"><span>คำแนะนำจากเจ้าของ</span><p>{stay.guardianCareInstruction ?? "ไม่มีคำแนะนำจากเจ้าของที่อนุญาตให้ใช้กับการเข้าพักนี้"}</p></div>
