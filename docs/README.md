...
Status: **CANONICAL / BE1 IDENTITY-BUSINESS-BRANCH + BE2 CUSTOMER-PET + BE3 BOOKING-CALENDAR IMPLEMENTED LOCALLY / BF1–BF12 UI FROZEN**
...
| [BACKEND_READINESS](./BACKEND_READINESS.md) | BE0 persistence/derived-state map plus BE1–BE3 schema, application boundaries, scope/concurrency enforcement and current backend boundary |
...
- **BF10–BF12 is implemented locally:** shared Business/Branch Settings at `/business/settings`, capability-gated Daycare at `/business/daycare`, and derived CRM/retention within existing Customers routes. The repository has **34 `page.tsx` entries: 30 active local routes, 3 redirects and 1 legacy demo**. Final test/browser results are owned by [VALIDATION](./VALIDATION.md), not implied by this implementation checkpoint.
- **Split source of truth is explicit:** Business text profile and Branch capabilities/hours are durable BE1 data; Customer/Pet relationship data is durable BE2 data; Booking planning and its minimal schedulable Resource projection are durable BE3 data. Grooming Jobs, Hotel Stays/rooms, Daycare Attendance, Team/HR, Intake, Inbox, Billing/Payment, Service Records and Reports remain in the browser-local compatibility envelope and reference durable IDs. CRM remains a read-time derivation, not a new database or persisted score.
...
- **BF-7 financial split:** `Service / Job / Stay / Attendance → Charge → Payment`; a Charge is not a Payment. `/business/billing` records browser-local, whole-THB Charges and Payments, derives unpaid/partial/paid status from allocations, attributes financial records to Branch, and supplies the same data to Customer history and Home revenue. Grooming, Hotel and Daycare completion never implies payment.
...
- **BE1 membership versus BF9 Team:** `/business/team` remains a browser-local operational directory; its displayed role/capability/availability data does not authorize requests. BE3 persists only an opaque `compatibilityStaffId` and planning availability windows on a Resource, not Person/Team truth. The separate BE1 membership/access records enforce Business/Branch scope server-side. Production authentication and granular operational permissions are not implemented.
...
- **Target production platform direction:** Cloudflare Worker/Vinext + D1 is implemented locally for BE1–BE3. **PRODUCTION: NOT DEPLOYED / NOT VERIFIED. PRODUCTION READY: NO.**
- **Strict Boundary:** BE1, BE2 and BE3 are implemented locally; BE4 is not started. Service execution, Guardian authority, Passport/Consent, Intake backend, Inbox/LINE, Billing/Payment, Reports, media/R2 and Consumer backends remain unimplemented. Consumer stays frozen; Team/HR persistence, payroll, real payments/accounting, Guardian LINE visibility and production deployment remain future work. `/workfiledesign` is untouched.
