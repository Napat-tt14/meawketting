...
Status: **REPOSITORY TRUTH — LOCAL FRONT-END PROTOTYPE / BUSINESS UX/UI RESPONSIVE RESET**
...
The current audit contains 28 `page.tsx` route entries: 24 active local routes, 3 compatibility redirects, and 1 legacy QR demo. The canonical route table is owned by [ROUTES](./ROUTES.md); Hotel / Boarding has no dedicated operations route yet.
...
| **Hotel / Boarding Operations (planned)** | Shared Calendar continues to represent date-range Hotel Bookings. Dedicated operations, occupancy, room assignment, daily care and Stay workflows have not started; the Business menu keeps a disabled `โรงแรม` button labeled `ยังไม่เปิดใช้` | Planned future milestone; no `/business/hotel` route |
| **Customers & Pets (BF-3)** | Responsive operational rows using the shared Business search field and 14px `BusinessSegmentedControl` filters (ทั้งหมด / มีนัดหมาย / ยังไม่มีนัด); zoned Customer detail with icon-only primary actions, inline name edit, Pet-section add action, neutral Customer/Pet avatars, mobile multiple-Pet snap/peek, progressive Passport/access disclosure, local add Customer/Pet, tags/notes, duplicate warning and Booking preselection | Browser-local session storage; no Customer/Guardian authority backend |
| **Inbox & Customer Communication (BF-4 boundary)** | `/business/inbox` readable desktop split/mobile full-task UI; Business-wide Customer conversation reuse; shared search; 14px `BusinessSegmentedControl` filters (ทั้งหมด / ยังไม่ได้อ่าน / กำลังใช้บริการ); safe Customer/Pet/Booking/Branch/Grooming context; local send/unread without browser-storage success copy; horizontal mobile quick replies; centered viewport-safe add-service dialog; query recovery and structured add-service request | Separate same-tab session storage; a Guardian-response helper remains only in local state/tests and can affect the linked Grooming estimate/add-on; no real messaging, attachments, notifications, Consumer Inbox, authorization, Booking, Charge, or Payment effect |
| **Shared Intake & Scanner (Phase E)** | Camera scan, neutral manual code recovery, QR validation, consent review, belongings logging, check-in completion and eligible Grooming Job handoff; fixture/interruption controls remain state/test-only | Implemented local prototype |
...
- The 2026-08-30 Business UX/UI reset audited the public Business entry, shell, Home, Calendar/Booking overlays, Customers/detail, Inbox, Scanner/Intake and Grooming across 320/375/390/430/768/820/1024/1200/1440px. It removed redundant operational overview copy and staff-facing development controls, established 16px body typography and 44px touch targets, and added restrained route/region/control motion with reduced-motion fallback. Hotel / Boarding remains a planned module with a visible disabled menu entry.
...
- BF1–BF5 operational UI is implemented without duplicating Customer/Pet/Booking records or rebuilding privacy/Booking logic: shared service identity, identity avatars, Home Spotlight, compact shell, Grooming capability navigation, retained planned navigation, row/timeline/schedule surfaces, and restrained cards/chips. Hotel / Boarding remains planned and is represented only by shared Calendar Booking support plus a disabled menu entry.
...
- Hotel / Boarding Operations is **PLANNED / NOT STARTED**. Shared Calendar can represent date-range Hotel Bookings, but no dedicated Hotel route, occupancy board, Hotel Stay state, room assignment/move history, daily-care workflow or Hotel-specific Intake handoff exists. The Business menu keeps a disabled planned button for the future module.
...
- Hotel / Boarding Operations remains deferred alongside Daycare Operations, Billing/Payments, Full CareProof, Full Incident Management, Backend, Database, Real Auth, LINE transport, and Cloudflare deployment. No next phase starts automatically.
