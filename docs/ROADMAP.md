...
| **BF-5: Grooming Operations Foundation** | **COMPLETE — LOCAL PROTOTYPE** | Capability-aware `/business/grooming` Today board; Booking-planning and Grooming Service Job-execution stay distinct; Pet-specific Job lifecycle, shared Customer/Pet/Resource/Intake/Inbox references, board/status controls, and lightweight completed-service history. At that milestone, Hotel/Daycare execution was still deferred; BF-6 subsequently adds the local Hotel foundation. |
| **BF-6: Hotel / Boarding Operations Foundation** | **COMPLETE — LOCAL PROTOTYPE** | Capability-aware `/business/hotel` execution dashboard; Booking-planning and Hotel Stay-execution remain distinct; continuous room/zone occupancy, guarded room assignment/date changes, room-move history, per-Pet daily care, attention, and shared Intake/Home/Customer/Inbox/Calendar references; no overbooking override, Billing, full CareProof, full Incident Management, backend, or LINE transport |
...
6. **Hotel / Boarding Operations Foundation (BF-6)** — **COMPLETE — LOCAL PROTOTYPE**: `/business/hotel` is a capability-aware execution surface, not another Calendar. It creates/links Pet-specific Hotel Stays from Hotel Bookings, preserves Booking planning status, renders continuous room/zone occupancy spans, blocks local overlap/capacity conflicts, keeps room-move history, and stores per-Pet daily-care completion. Intake, Home, Customer history and Inbox reuse shared records; policies for overbooking, room sharing and cross-Branch transfer remain open.
...
Completion of BF-6 Hotel does **NOT** authorize starting Daycare Operations, Billing/Payments, full CareProof, full Incident Management, Backend, Production, LINE Login, LINE Mini App, or Consumer work automatically. Overbooking overrides, room-sharing policy, waitlist and cross-Branch Hotel transfer remain open and are not authorized by this foundation. Each milestone requires a separate Product Owner approval and execution plan. Consumer remains paused until a Business workflow milestone requires a Guardian-side touchpoint.
