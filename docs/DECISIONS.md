...
| D-43 | Service Job remains the shared direction for module work extended as Grooming Job, future Hotel Stay, Daycare Visit, etc.; module UX and lifecycles remain service-specific. BF-5 locally implements Grooming Service Jobs; Hotel execution remains planned. |
...
| D-56 | Business navigation exposes live Home, Calendar, Customers, and BF-4 Messages plus Scanner in the Header. Grooming is live only for capable active Branches; Hotel, Daycare, Finance, Reports, Team and Settings remain visible as planned/disabled controls with no fake routes |
| D-57 | The browser-local active Business/Branch context drives Home summaries and visible Branch-enabled service rows together; the capability-gated Grooming route is live only for its capable Branch, while Hotel remains a disabled planned row and modules not enabled for the Branch remain absent |
...
- Hotel / Boarding operations is intentionally not started. Shared Calendar date-range Booking support remains browser-local planning data; occupancy, room assignment/moves, daily care, Hotel Stay state and Hotel-specific Intake behavior require a future approved milestone.
