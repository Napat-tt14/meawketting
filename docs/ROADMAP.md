...
| **BF-5: Grooming Operations Foundation** | **COMPLETE — LOCAL PROTOTYPE** | Capability-aware `/business/grooming` Today board; Booking-planning and Grooming Service Job-execution stay distinct; Pet-specific Job lifecycle, shared Customer/Pet/Resource/Intake/Inbox references, board/status controls, and lightweight completed-service history. Hotel / Boarding and Daycare execution remain deferred. |
| **Hotel / Boarding Operations** | **PLANNED — NOT STARTED** | Shared Calendar keeps date-range Hotel Bookings; dedicated Hotel execution/occupancy, room assignment/moves, daily care and Stay workflow are deferred. A disabled planned menu item remains visible and no `/business/hotel` route exists. |
...
6. **Hotel / Boarding Operations** — **PLANNED — NOT STARTED**: Shared Calendar retains date-range Hotel Bookings. A dedicated Hotel route, occupancy, room assignment/moves, daily care and Stay workflow will be designed later; the disabled menu item communicates the planned capability without creating a working surface.
