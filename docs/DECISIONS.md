...
### BF-6 Hotel / Boarding Operations Foundation — 2026-08-28

| ID | Decision |
|---|---|
| D-120 | **Hotel / Boarding Operations Foundation is authorized and implemented as a local prototype only.** `/business/hotel` is live only for an active Branch with Hotel enabled. It is an execution, occupancy and daily-care surface; Calendar remains the shared Booking planning surface. This narrowly supersedes the historical “do not start Hotel” boundary and does not authorize Daycare, Billing, full CareProof, full Incident Management, Backend, deployment, or Consumer work. |
| D-121 | **Booking and Hotel Stay remain distinct.** A Hotel Booking plans the date range; a Pet-specific `PrototypeHotelStay` in the shared browser-local Business envelope owns Hotel execution. It references the existing Booking, Customer, Pet, Branch and Resource records rather than copying them. Booking planning status is never reused as the Hotel Stay lifecycle, and the local implementation does not require a generic Service Job migration. |
| D-122 | **Hotel availability blocks conflict by default.** Before local room/zone assignment, move, extension, shortening, or date change, overlap and Resource capacity are evaluated. A conflict retains the prior state and receives specific recovery copy. There is no overbooking override/permission flow and no waitlist. Whether overbooking is ever allowed, room-sharing policy, room type/capacity policy, and move authority remain OPEN. |
| D-123 | **Room moves and daily care retain operational evidence without becoming compliance systems.** A move appends timestamped prior/next room-or-zone history plus optional Business note; it does not overwrite the former room. Per-Pet daily-care tasks retain local completion time and available staff identity, while Guardian instructions, Business notes, and daily tasks stay separate. This is not medical management, housekeeping workforce scheduling, Full CareProof, or Full Incident Management. |
| D-124 | **Hotel reuses Business Core boundaries.** Capability controls both Hotel navigation and direct route behavior. Shared Intake may target a valid linked Stay; Home, Calendar, Customers/Pets, Resources, and Inbox use shared references. A Hotel+Grooming journey shares Customer/Pet context but preserves independent Stay/Job lifecycles. Business can request an add-on through the existing local Inbox foundation but cannot self-approve; no LINE API or real transport is introduced. Passport data remains gated by active Business/Branch/scope/duration consent, including expiry/revoke. |
| D-125 | **Hotel visual and responsive treatment is operational.** Bed/Sky identifies the module only; semantic status/attention remains distinct. Desktop/tablet uses a scannable room/zone-by-date occupancy board with continuous spans; mobile uses date/list/detail controls rather than squeezing a wide grid. Pet visual anchors, accessible labels/focus, a non-drag room-change path, restrained 180–300ms/reduced-motion interaction, and Light/Warm White-only Design System rules apply. |
...
| D-43 | Service Job remains the shared direction for module work extended as Grooming Job, Hotel Stay, Daycare Visit, etc.; module UX and lifecycles remain service-specific. BF-5 locally implements Grooming Service Jobs; BF-6 keeps Hotel Stay as a distinct module-specific local slice while the broader generic migration remains future architecture work. |
...
| D-56 | Business navigation exposes live Home, Calendar, Customers, and BF-4 Messages plus Scanner in the Header. Grooming- and Hotel-enabled service modules are live only for capable active Branches; Daycare, Finance, Reports, Team and Settings remain visible as planned/disabled controls with no fake routes |
| D-57 | The browser-local active Business/Branch context drives Home summaries and visible Branch-enabled service rows together; a capability-gated Grooming/Hotel route is live only for its capable Branch, while modules not enabled for the Branch remain absent |
...
- BF-6 Hotel Stays, room/zone assignment, move history, daily-care tasks/completion, instructions, attention, and lightweight completed-stay history are same-tab browser-local data in the shared Business envelope. The fixture rooms/zones, dates, staff labels and care schedule are demonstration aids, not a room-inventory, medical, staffing, capacity-policy, checkout-payment, or SLA decision.
- **BF-6 prototype conflict treatment:** the local default blocks an overlap/capacity conflict before it writes. This is a reversible safety assumption, not a policy decision that overbooking is forbidden in production. Overbooking authority (OQ-BF11), room sharing/capacity (OQ-BF09), room-move detail/authority (OQ-BF10), waitlist (OQ-BF12), cancellation/no-show (OQ-BF14), daily-care requirements (OQ-BF25), and cross-Branch transfer (OQ-BF05/OQ-BF27) remain OPEN.
- Hotel room/resource names are fixture/config references only. The prototype does not encode a universal one-Pet-per-room rule or automatic shared-room admission.
