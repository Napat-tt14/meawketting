...
export class D1Be5Repository {
  constructor(readonly database: D1DatabaseLike, readonly authorityMode: "dev-test" | "verified-provider" = "verified-provider") {}
  ...
  async access(grant: GrantRow, now: string): Promise<AccessView> {
    const events = await this.rows<{ id: string; kind: string; occurred_at: string; actor_person_id: string }>(
      "SELECT id,kind,occurred_at,actor_person_id FROM access_events WHERE business_id=? AND branch_id=? AND grant_id=? ORDER BY occurred_at DESC,rowid DESC LIMIT 100", grant.business_id, grant.branch_id, grant.id);
  ...
  private grantPredicate(active: boolean) {
    return `EXISTS(SELECT 1 FROM access_grants g JOIN consents c ON c.id=g.consent_id AND c.business_id=g.business_id AND c.branch_id=g.branch_id
      JOIN pet_authorities a ON a.id=c.authority_id AND a.pet_id=c.pet_id JOIN persons p ON p.id=a.person_id
      WHERE g.business_id=? AND g.branch_id=? AND g.id=? AND g.revision=? AND g.revoked_at IS NULL AND g.expires_at>max(?,strftime('%Y-%m-%dT%H:%M:%fZ','now'))
  ...
  guard(grant: GrantRow, now: string, id: string, active = true) { return this.database.prepare(`INSERT INTO backend_guards(id,allowed,version_ok) SELECT ?,1,${this.grantPredicate(active)}`).bind(id, ...this.grantBindings(grant, now)); }
  ...
  async result(row: IntakeRow, grant: GrantRow, now: string): Promise<IntakeResult> {
    ...
    const corrections = passport ? await this.rows<{ id: string; topic: "name" | "species" | "passport-reference"; current_value: string; suggested_value: string; note: string; created_at: string }>(
      "SELECT * FROM intake_corrections WHERE business_id=? AND branch_id=? AND intake_id=? ORDER BY created_at DESC,rowid DESC LIMIT 1", row.business_id, row.branch_id, row.id) : [];
  ...
  async write(grant: GrantRow, context: AuthorizedMutation, receipt: MutationReceipt, statements: D1PreparedStatementLike[], active = true, action = receipt.command) {
