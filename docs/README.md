...
Status: **CANONICAL / LOCAL FRONT-END PROTOTYPE / BF-6 HOTEL OPERATIONS FOUNDATION**
...
- **Strict Boundary:** Consumer visual redesign remains paused. BF-6 is a lightweight local Hotel foundation only; do not start BF-7, Daycare, Billing, CareProof, AI Product features, or any other next phase automatically. Backend, database, real auth, real payments, and production deployment remain future work. Advanced room pricing/policy, waitlist, cross-Branch transfer, inventory, medical management, full CareProof, full Incident Management, and LINE transport remain OPEN or NOT IMPLEMENTED as documented by their owner files.
